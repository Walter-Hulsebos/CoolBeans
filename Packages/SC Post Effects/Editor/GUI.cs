...
namespace SCPE
{
    public class SCPE_GUI : Editor
    {
        ...
        public static void DisplaySetupWarning<T>(ref bool state) { DisplayDualInstallationError(); }
        ...
        {
#if URP
            //Built-in RP editor does not allow access to the SerializedObject target
            DisplayShaderReferenceError(so);
...
"               \n\nUninstall the package not appropriate for the chosen render pipeline...", MessageType.Error);
        ...
        public static void DisplayShaderReferenceError(SerializedObject so)
        {
            SerializedProperty prop = so.FindProperty("shader");

            if (prop.objectReferenceValue == null)
            {
                EditorGUILayout.HelpBox("Shader isn't referenced, effect will not function in a build!" +
                                        "\n\nThis may be because this component was added before updating to v2.3.0 of the asset", MessageType.Error);
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    if (GUILayout.Button(new GUIContent("Fix", EditorGUIUtility.IconContent("d_tab_next").image), GUILayout.Width(60)))
                    {
                        Installer.Shaders.SerializeShader(so.targetObject);
                    }
                    GUILayout.Space(8);
                }
                GUILayout.Space(11);
