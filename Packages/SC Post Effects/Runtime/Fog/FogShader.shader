Shader "Hidden/SC Post Effects/Fog"
{
	...
	{
		half4 color = SAMPLE_TEXTURE2D_X(_BloomTex, Clamp, uv);
		return bloom + color;
	}
	...
	float4 FragBlend(Varyings input) : SV_Target
	{
		...
		float4 fogColor = ComputeFog(worldPos, UV, linearDepth);
	...
	float4 FragBlendScattering(Varyings input) : SV_Target
	{
		...
		#ifndef URP
		half4 bloom = SAMPLE_TEXTURE2D_X(_BloomTex, Clamp, UV) * _ScatteringParams.y;
